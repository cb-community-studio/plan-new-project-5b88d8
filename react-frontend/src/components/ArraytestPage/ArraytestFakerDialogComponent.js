...
import arraytestFakerFactory from "./arraytestFakerFactory";
...
const ArraytestFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = arraytestFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("arraytest").create(elem));
...
export default connect(null, mapDispatch)(ArraytestFakerDialogComponent);
